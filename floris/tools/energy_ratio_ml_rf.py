...
def energy_ratio(ref_pow_base, test_pow_base, ws_base,
    ...
    # Normalize
    ref_pow_base_norm = ref_pow_base / NORM_POWER
    ref_pow_con_norm = ref_pow_con / NORM_POWER
    ws_base_norm = ws_base / NORM_WS
    ws_con_norm = ws_con/NORM_WS

    X_base = np.column_stack([ref_pow_base_norm,ws_base_norm])
    X_con = np.column_stack([ref_pow_con_norm,ws_con_norm])
    X_all = np.concatenate([X_base,X_con])

    model = RandomForestRegressor()
    model.fit(X_base, test_pow_base)
    pred_base = model.predict(X_all)
    model.fit(X_con, test_pow_con)
    pred_con = model.predict(X_all)

    # Weighted sums
    weight_sum_ref = np.sum(ref_pow_base) + np.sum(ref_pow_con)
    weight_sum_test_base = np.sum(pred_base)
    weight_sum_test_con = np.sum(pred_con)
