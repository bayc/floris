...
class Yaw():
    ...
    """
    def __init__(self, fi, minimum_yaw_angle=0.0, maximum_yaw_angle=25.0, x0=None, include_unc=False, unc_pmfs=None, unc_options=None, wdir=None, wspd=None):
        """
        ...
        if x0 is not None:
            ...
            self.x0 = [turbine.yaw_angle for turbine in self.fi.floris.farm.turbine_map.turbines]
    ...
    def add_var_group(self, optProb):
        optProb.addVarGroup('yaw', self.nturbs, type='c',
    ...
    def plot_yaw_opt_results(self, sol):
        ...
        ax.set_title('Optimal Yaw Offsets for U = '+str(self.wspd[0])+' m/s, Wind Direction = '+str(self.wdir[0])+'$^\circ$')
    ...
    def print_power_gain(self, sol):
        ...
        self.fi.calculate_wake(yaw_angles=0.)
        power_baseline = self.fi.get_farm_power()

        self.fi.calculate_wake(yaw_angles=yaw)
        power_opt = self.fi.get_farm_power()

        pct_gain = 100.*(power_opt - power_baseline)/power_baseline
