...
class CoDesignOptimizationPyOptSparse(LayoutOptimization):
    ...
    def parse_opt_vars(self, varDict):
        ...
        self.yaw = [
            varDict["yaw"][i * self.nturbs : i * self.nturbs + self.nturbs]
            for i in range(len(self.wdir))
        ]
        self.yaw = self._unnorm(np.reshape(self.yaw, (len(self.wdir), len(self.wspd), self.nturbs)), self.yawmin, self.yawmax)
        print(self.yaw)

    def parse_sol_vars(self, sol):
        self.x = self._unnorm(sol.getDVs()["x"], self.xmin, self.xmax)
        self.y = self._unnorm(sol.getDVs()["y"], self.ymin, self.ymax)
        self.yaw = self._unnorm(sol.getDVs()["yaw"], self.yawmin, self.yawmax)
    ...
    def plot_layout_opt_results_with_flow(self, sol, file_name, wd, ws):
        """
        Method to plot the old and new locations of the layout opitimization.
        """
