...
def calculate_wind_speed(cross_plane, x1_loc, x2_loc, rotor_radius):
    """
    ...
    Args:
        ...
        rotor_radius (float): radius from point of interest to consider

    Returns:
        (float): effective wind speed
    """
    # Make a distance column
    distance = np.sqrt((cross_plane.df.x1 - x1_loc)**2 +
                       (cross_plane.df.x2 - x2_loc)**2)

    # Return the mean wind speed
    return np.cbrt(np.mean(cross_plane.df.u[distance < rotor_radius]**3))
...
def wind_speed_profile(cross_plane,
    ...
    v_array = np.array([calculate_wind_speed(cross_plane,x1_loc, x2_loc, rotor_radius) for x1_loc in x1_locs])
...
def calculate_power(cross_plane,
    ...
    Args:
        ...
        x1_loc (float): x1-coordinate of point of interest.
        x2_loc (float): x2-coordinate of point of interest.
        rotor_radius (float): Radius of wind turbine rotor.
    ...
    """
    # Compute the ws
    ws = calculate_wind_speed(cross_plane, x1_loc, x2_loc, rotor_radius)

    # Compute the cp
    cp_value = np.interp(ws, ws_array, cp_array)

    #Return the power
    return 0.5 * air_density * (np.pi * rotor_radius**2) * cp_value * ws**3


def get_power_profile(cross_plane, x2_loc, ws_array, cp_array, rotor_radius, air_density=1.225, resolution=100, x1_locs=None):

        if x1_locs is None:
            x1_locs = np.linspace(
                min(cross_plane.df.x1),
                max(cross_plane.df.x1),
                resolution
            )

        # Get the wind speed profile
        x1_locs, v_array = wind_speed_profile(
            cross_plane,
            rotor_radius,
            x2_loc,
