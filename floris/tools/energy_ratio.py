...
def calculate_balanced_energy_ratio(
    ...
    for i, wind_direction_bin in enumerate(wind_direction_bins):
        ...
        reference_power_baseline_wd = reference_power_baseline[
            wind_dir_mask_baseline]
        test_power_baseline_wd = test_power_baseline[wind_dir_mask_baseline]
        wind_speed_array_baseline_wd = wind_speed_array_baseline[
            wind_dir_mask_baseline]
        ...
        test_power_controlled_wd = test_power_controlled[
            wind_dir_mask_controlled]
        ...
        wind_speed_array_baseline_wd = wind_speed_array_baseline_wd.round(
        ).astype(int)
        wind_speed_array_controlled_wd = wind_speed_array_controlled_wd.round(
        ).astype(int)

        # compute the energy ratio
        ratio_array_base[i], ratio_array_con[i], diff_array[i], p_change_array[
            i], counts_ratio_array_base[i], counts_ratio_array_con[
                i], counts_diff_array[i], counts_p_change_array[
        ...
        for i_bs in range(n_boostrap):
            ...
            if wind_direction_bin_p_overlap > 1:
                ...
                ind_bs = np.random.choice(
                    len(reference_power_baseline_wd),
                    size=len(reference_power_baseline_wd))
            reference_power_binned_baseline = reference_power_baseline_wd[
                ind_bs]
            ...
            if wind_direction_bin_p_overlap > 1:
                ...
                ind_bs = np.random.choice(
                    len(reference_power_controlled_wd),
                    size=len(reference_power_controlled_wd))
            reference_power_binned_controlled = reference_power_controlled_wd[
                ind_bs]
            test_power_binned_controlled = test_power_controlled_wd[ind_bs]
            wind_speed_binned_controlled = wind_speed_array_controlled_wd[
                ind_bs]

            # compute the energy ratio
            ratio_base_bs[i_bs], ratio_con_bs[i_bs], diff_bs[
                i_bs], p_change_bs[i_bs], _, _, _, _ = energy_ratio(
                    reference_power_binned_baseline,
                    test_power_binned_baseline, wind_speed_binned_baseline,
                    reference_power_binned_controlled,
                    test_power_binned_controlled, wind_speed_binned_controlled)
...
def plot_energy_ratio(reference_power_baseline,
    ...
    if plot_simple:
        ...
        ax.plot(wind_direction_bins,
                ...
                ls='-',
                marker='.')
        ax.fill_between(wind_direction_bins,
                        ...
                        label='_nolegend_')
        if show_count:
            ax.scatter(wind_direction_bins,
                       ratio_array_base,
...
def energy_ratio_ws(ref_pow_base,
                    test_pow_base,
                    wd_base,
                    ref_pow_con,
                    test_pow_con,
...
def calculate_balanced_energy_ratio_ws(reference_power_baseline,
    ...
    for i, wind_speed_bin in enumerate(wind_speed_bins):
        ...
        reference_power_baseline_ws = reference_power_baseline[
            wind_speed_mask_baseline]
        test_power_baseline_ws = test_power_baseline[wind_speed_mask_baseline]
        wind_dir_array_baseline_ws = wind_direction_array_baseline[
            wind_speed_mask_baseline]

        reference_power_controlled_ws = reference_power_controlled[
            wind_speed_mask_controlled]
        test_power_controlled_ws = test_power_controlled[
            wind_speed_mask_controlled]
        wind_dir_array_controlled_ws = wind_direction_array_controlled[
            wind_speed_mask_controlled]

        if (len(reference_power_baseline_ws) == 0) or (
                len(reference_power_controlled_ws) == 0):
            continue
        ...
        ratio_array_base[i], ratio_array_con[i], diff_array[i], p_change_array[
            i], counts_ratio_array_base[i], counts_ratio_array_con[
                i], counts_diff_array[i], counts_p_change_array[
                    i] = energy_ratio_ws(reference_power_baseline_ws,
                                         test_power_baseline_ws,
                                         wind_dir_array_baseline_ws,
                                         reference_power_controlled_ws,
                                         test_power_controlled_ws,
                                         wind_dir_array_controlled_ws,
        ...
        for i_bs in range(n_boostrap):
            ...
            # random resampling w/ replacement
            ind_bs = np.random.randint(len(reference_power_baseline_ws),
                                       size=len(reference_power_baseline_ws))
            reference_power_binned_baseline = reference_power_baseline_ws[
                ind_bs]
            test_power_binned_baseline = test_power_baseline_ws[ind_bs]
            wind_dir_binned_baseline = wind_dir_array_baseline_ws[ind_bs]

            ind_bs = np.random.randint(len(reference_power_controlled_ws),
                                       size=len(reference_power_controlled_ws))
            reference_power_binned_controlled = reference_power_controlled_ws[
                ind_bs]
            ...
            # compute the energy ratio
            ratio_base_bs[i_bs], ratio_con_bs[i_bs], diff_bs[
                i_bs], p_change_bs[i_bs], _, _, _, _ = energy_ratio_ws(
                    reference_power_binned_baseline,
                    test_power_binned_baseline,
                    wind_dir_binned_baseline,
                    reference_power_binned_controlled,
                    test_power_binned_controlled,
                    wind_dir_binned_controlled,
                    use_absolutes=use_absolutes,
                    use_mean=use_mean)
...
def plot_energy_ratio_ws(reference_power_baseline,
    ...
    if use_absolutes:
        r_ratio_array_base, r_lower_ratio_array_base, r_upper_ratio_array_base, r_counts_ratio_array_base, r_ratio_array_con, r_lower_ratio_array_con, r_upper_ratio_array_con, r_counts_ratio_array_con, r_diff_array, r_lower_diff_array, r_upper_diff_array, r_counts_diff_array, r_p_change_array, r_lower_p_change_array, r_upper_p_change_array, r_counts_p_change_array = calculate_balanced_energy_ratio_ws(
            test_power_baseline,
            reference_power_baseline,
            wind_speed_array_baseline,
            wind_direction_array_baseline,
    ...
    if plot_simple:
        ...
        ax.plot(wind_speed_bins,
                ...
                ls='-',
                marker='.')
        ax.fill_between(wind_speed_bins,
                        lower_ratio_array_base,
                        upper_ratio_array_base,
                        alpha=0.3,
                        color=base_color,
                        label='_nolegend_')
        ax.scatter(wind_speed_bins,
                   ratio_array_base,
                   s=counts_ratio_array_base,
                   label='_nolegend_',
                   color=base_color,
    ...
    if use_absolutes:  #show the references as well
        if plot_simple:
            ...
            ax.plot(wind_speed_bins,
                    ...
                    marker='.')
            ax.fill_between(wind_speed_bins,
                            r_lower_ratio_array_base,
                            r_upper_ratio_array_base,
                            alpha=0.3,
                            color='k',
                            label='_nolegend_')
            ax.scatter(wind_speed_bins,
            ...
            ax.plot(wind_speed_bins,
                    ...
                    marker='.')
            ax.fill_between(wind_speed_bins,
                            r_lower_ratio_array_con,
                            r_upper_ratio_array_con,
                            alpha=0.3,
                            color='k',
                            label='_nolegend_')
            ax.scatter(wind_speed_bins,
