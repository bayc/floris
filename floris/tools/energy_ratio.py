...
def energy_ratio(
    ref_pow_base, test_pow_base, ws_base, ref_pow_con, test_pow_con, ws_con
):
    """
    ...
    p_change = 100.0 * ratio_diff / ratio_base
...
def calculate_balanced_energy_ratio_ws(
    reference_power_baseline,
    test_power_baseline,
    wind_speed_array_baseline,
    wind_direction_array_baseline,
    ...
    # Compute binning radius (is this right?)
    wind_speed_bin_radius = (
        (1.0 + wind_speed_bin_p_overlap / 100.0)
        * (wind_speed_bins[1] - wind_speed_bins[0])
        / 2.0
    )
    ...
    for i, wind_speed_bin in enumerate(wind_speed_bins):
        ...
        for i_bs in range(n_boostrap):
            ...
            ind_bs = np.random.randint(
                len(reference_power_controlled_ws),
...
def plot_energy_ratio_ws(
    ...
    (
        ...
        upper_p_change_array,
        counts_p_change_array,
    ) = calculate_balanced_energy_ratio_ws(
        reference_power_baseline,
        test_power_baseline,
        wind_speed_array_baseline,
        wind_direction_array_baseline,
    ...
    if use_absolutes:
        (
            ...
            r_counts_ratio_array_base,
            r_ratio_array_con,
            r_lower_ratio_array_con,
            r_upper_ratio_array_con,
            r_counts_ratio_array_con,
            r_diff_array,
            r_lower_diff_array,
            r_upper_diff_array,
            r_counts_diff_array,
            r_p_change_array,
            r_lower_p_change_array,
            r_upper_p_change_array,
            r_counts_p_change_array,
        ) = calculate_balanced_energy_ratio_ws(
            test_power_baseline,
            reference_power_baseline,
            wind_speed_array_baseline,
    ...
    if plot_simple:
        ax.plot(
            wind_speed_bins, diff_array, label=label_pchange, color=con_color, ls="--"
        )
